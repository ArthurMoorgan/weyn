...
// Ambient WebGL hero background (React Bits' "Ferrofluid", adapted) — a
// magnetic-fluid-style shader, tinted to Weyn's own brand colors rather
// than the component's generic demo palette. Lives in its own chunk (see
// WaitlistLanding.tsx's dynamic import) since ogl + a fragment shader has
// no business being in the main app bundle — nobody outside this one
// marketing page needs it.
const MAX_COLORS = 8;
...
function prepColors(input?: string[]) {
  const base = (input && input.length ? input : ["#0E0E10", "#7C3AED", "#9D7BFF"]).slice(0, MAX_COLORS);
...
export default function Ferrofluid({
  ...
  useEffect(() => {
    ...
    const loop = (t: number) => {
      ...
      if (!paused) {
        try {
          ...
          // context lost or similar transient GL error — skip this frame
        }
      }
    };
    rafId = requestAnimationFrame(loop);
    ...
    return () => {
      ...
      // frees the compiled shader/buffers — ogl's Renderer has no
      // equivalent teardown method, only Program/Geometry do
      program.remove();
      geometry.remove();
      const lossExt = renderer.gl.getExtension("WEBGL_lose_context") as { loseContext(): void } | null;
      lossExt?.loseContext();
    };
