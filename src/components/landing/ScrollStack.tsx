...
// React Bits' "ScrollStack", ported to TS as-is (see SplitText.tsx's
// comment). Used for the landing page's feature cards — each one pins and
// scales down as the next scrolls over it.
...
export default function ScrollStack({
  ...
  // "useWindowScroll" tracks the page's real scroll container, which in
  // this app is `document.body` everywhere (see index.css's html/body/#root
  // height:100% setup) — not `window`/documentElement, which never fires a
  // scroll event here at all.
  const getScrollData = useCallback(() => {
    if (useWindowScroll) {
      return { scrollTop: document.body.scrollTop, containerHeight: window.innerHeight };
    }
  ...
  const setupLenis = useCallback(() => {
    if (useWindowScroll) {
      // Deliberately NOT using Lenis here: it would need to wrap
      // `document.body` (the app's real scroll container, see
      // getScrollData's comment) to intercept wheel/touch — i.e. take over
      // scroll smoothing for the ENTIRE page, not just this section. Too
      // large a blast radius for one landing-page component. A plain native
      // scroll listener gets the same stacking effect without touching how
  ...
  useLayoutEffect(() => {
    ...
    return () => {
      ...
      if (useWindowScroll) document.body.removeEventListener("scroll", handleScroll);
      stackCompletedRef.current = false;
      cardsRef.current = [];
