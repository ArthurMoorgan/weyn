...
// Cycling word/phrase (React Bits' "RotatingText", ported to TS). Used in
// the landing page's subheading to cycle through what Weyn is for
// (discover / host / reserve) without needing three separate headlines.
...
type WordGroup = { characters: string[]; needsSpace: boolean };
...
const RotatingText = forwardRef<RotatingTextHandle, RotatingTextProps>((props, ref) => {
  ...
  const splitIntoCharacters = (text: string): string[] => {
    // Intl.Segmenter isn't in this project's configured TS lib target yet
    // (widely supported at runtime — Baseline since 2024) — access it via
    // an untyped lookup rather than bumping the whole project's lib target
    // for one component.
    const IntlWithSegmenter = Intl as typeof Intl & { Segmenter?: new (locale: string, opts: { granularity: string }) => { segment(s: string): Iterable<{ segment: string }> } };
  ...
  const elements = useMemo<WordGroup[]>(() => {
