...
9. [x] **Ticket quantity stepper added** (2026-07-12, latest round) — the user attached fresh screenshots showing per-tier +/- quantity steppers on Event Detail's "Select tickets" panel; this app's `checkoutEvent`/`bookEvent`/`organizerPaymentCheckout` API calls already accepted a quantity param but every call site hardcoded `1`. Added `qty` state in `EventDetail.tsx`, wired it through all three call sites, rebuilt `.tier-opt` cards to match the reference (tinted+bordered selected state, name/availability/price header, stepper appears under the selected tier only), and updated the order-summary line to "N × Tier name". **Not yet visually verified live** — no event in the production DB currently has more than one ticket tier, so the tier-picker/stepper UI never renders on any real event page. Typecheck/build are clean; the code path is straightforward, but if you can, create or find a tiered event and eyeball it.
10. [ ] **Still not done — explicit user ask on this round**: "copy the EXACT layout" of the newly attached screenshots (Discover desktop, Event Detail desktop+mobile, Search & filters mobile, Ticket wallet mobile). Concretely still open:
   - Ticket wallet stub badge shows a generic "Confirmed" status pill; the reference shows the actual tier name ("GOLDEN CIRCLE", "CATEGORY 1") as a coral-tinted pill instead. Doing this properly needs the ticket list to carry tier-name data per booking, which `Tickets.tsx`/`Stub.tsx` don't currently fetch — would need a small API addition (or a booking-details fetch per ticket).
   - Search & filters mobile reference shows the active filter chips (This weekend / Music / AED 100–600) as a row directly under the search bar on the main search results view — check this matches `Explore.tsx`'s `active-filter-summary` exactly (it exists but wasn't re-diffed against this specific screenshot).
   - Desktop top nav in the reference is `weyn` wordmark + `Discover | Events | Venues` as three separate tab destinations, with search as its own icon-triggered page — this repo's IA is intentionally different (Discover contains an Events/Venues segmented toggle + inline search bar, per earlier redesign decisions this session) and wasn't changed, since collapsing that would undo deliberate earlier work without the user asking for that specific IA change. Flag this explicitly to the user if they want the nav itself restructured, rather than assuming.
